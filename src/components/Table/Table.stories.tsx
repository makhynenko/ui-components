...
const tableDataOptions = [
  ...
  { label: 'Data is loading - custom', value: 'loadingDataCustom', disabled: false },
    ...
    <div>
      ...
      <div style={{ height: '400px', overflow: 'hidden', marginBottom: '16px' }}>
        <Table<User>
          ...
          onSelectRow={(record) => {
            alert(
              `On row click you got all data about this row. For example: Name of selected row is ${record.name}.`
            );
            console.log(record);
          }}
        />
...
//   onScroll?: (event: React.UIEvent) => void; -ignore for now
//?  fetchMore?: () => void; - do not understand how should it work
//?  hasMore?: boolean; - do not understand how should it work

//   expandableContent?: (record: ITableRecord<T>) => JSX.Element;
//   expandedRowKeys?: string[];

//!   onSort?: (sorting: ITableSort) => void;
//!   dataTestId?: string; - need to be removed
//!   sorting?: ITableSort; - need to be removed
// }
